...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    r = [i for i in paragraphs if select(i)]
    if k < len(r):
        return r[k]
    else:
        return ''
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
    def about_sub(string):
        y = split(remove_punctuation(lower(string)))
        for i in subject:
            for j in y:
                if i == j:
                    return True
...
def wpm(typed, elapsed):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 4
    num_of_group = len(typed) / 5
    return num_of_group * 60 / elapsed
...
def furry_fixes(typed, source, limit):
    ...
    if len(typed) == 0:
        ...
        return len(typed) 
    elif typed[0] == source[0] and limit >= 0:#相等，限制大于等于-1，返回值不变，limit不变
        return furry_fixes(typed[1:], source[1:], limit)
    elif typed[0] != source[0] and limit > 0:#不等，限制大于0，返回值+1，limit-1
...
def time_per_word(words, timestamps_per_player):
    ...
    # END PROBLEM 9
    times = []
    for innerlist in timestamps_per_player:
        n = []
        for i in range(0,len(innerlist)-1):
            n = n + [innerlist[i+1]-innerlist[i]]
        times = times + [n]
...
def time_per_word_match(words, timestamps_per_player):
    ...
    # END PROBLEM 10
    class Match:
        word, time = match(time_per_word(words,timestamps_per_player))
